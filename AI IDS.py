...
LOG_HEADERS = ['timestamp','protocol','flag','src_bytes','dst_bytes',
               ...
               'label','risk_score','risk_level','alert_reasons','action']
...
def predict_packet(pkt, mdl):
    ...
    action = 'Blocked' if score >= ALERT_THRESHOLD else 'Allowed'
...
def write_alert(entry):
    with open(ALERT_LOG, 'a') as f:
        f.write(f"[{entry['timestamp']}] [{entry['action'].upper()}] {entry['risk_level'].upper()} | "
...
<div class="card">
  <div class="logo">
    ...
    <h1>NetGuard IDS/IPS</h1>
    <p class="sub">AI-Powered Intrusion Detection & Prevention</p>
  </div>
...
<title>NetGuard IDS/IPS Dashboard</title>
...
.action-badge{display:inline-block;padding:2px 7px;border-radius:3px;font-size:.62rem;font-weight:600;}
.action-allowed{background:rgba(0,255,136,.15);color:var(--green);border:1px solid rgba(0,255,136,.3);}
.action-blocked{background:rgba(255,61,107,.15);color:var(--accent2);border:1px solid rgba(255,61,107,.3);}
...
<div class="stats">
  ...
  <div class="stat yellow">
    <div class="stat-lbl">Packets Blocked</div>
    <div class="stat-val yellow" id="s-blocked">0</div>
    ...
    <div class="t-wrap">
      <table>
        <thead><tr>
          ...
          <th>Label</th><th>Score</th><th>Level</th><th>Action</th><th>Alerts</th>
        </tr></thead>
...
<script>
// ── State
const stats = {total:0, normal:0, attack:0, blocked:0};
...
function addRow(d){
  ...
  const ac = d.action === 'Allowed' ? 'action-allowed' : 'action-blocked';
  tr.innerHTML = `
    ...
    <td><span class="action-badge ${ac}">${d.action}</span></td>
    <td>${alerts}</td>
  `;
...
function addAlert(d){
  ...
  div.innerHTML = `
    ...
    <div class="alert-body">
      <div class="alert-title">[${d.action.toUpperCase()}] ${d.label} — ${d.risk_level}</div>
...
es.addEventListener('packet', e => {
  ...
  if(d.action === 'Blocked') { stats.blocked++; addAlert(d); }
  addRow(d);
  updateStats();
  updateCharts(d);
});
...
# Global model state
MODEL  = {}
live_stats  = {'total':0,'normal':0,'attack':0,'blocked':0}
lock   = threading.Lock()
